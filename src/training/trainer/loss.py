...
def compute_loss(
    ...
    Handles both one-hot targets (from Mixup/Cutmix) and class indices.

    Args:
        logits (Tensor): Model outputs of shape :math:`(B, C)` or :math:`(B, num\_classes)`
        targets (Tensor): Target labels, either one-hot of shape :math:`(B, C)` or
            class indices of shape :math:`(B,)`
        reduction (str): Loss reduction method. Options: ``"mean"``, ``"sum"``, ``"none"``.
            Default: ``"mean"``
    ...
    Examples::
        ...
        >>> loss, components = compute_loss(logits, targets)
        >>> loss.item()
        2.345
    """
