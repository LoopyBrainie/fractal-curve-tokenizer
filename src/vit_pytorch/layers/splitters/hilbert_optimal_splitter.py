...
class HilbertOptimalSplitter(nn.Module, CoreSplitter):
    ...
    def forward(
        ...
        # I-OPT: 保持 tensor 传递给 _sparse_select，避免 .item() GPU-CPU 同步
        K_tensor = K_float.long().clamp(self.K_min, self._current_K)

        # 稀疏选择（传入 epoch 以支持自适应 α 调度）
        selected_mask, probs = self._sparse_select(logits, K_tensor, hard=hard, epoch=epoch)
        ...
        if len(selected_indices[1]) == 0:
            # 至少选择一个 - 使用 top-k
            # FIX: K_tensor 替代 K（K 未定义）
            _, topk_idx = torch.topk(probs[0], K_tensor.clamp(max=probs.shape[1]), dim=-1)
            # 使用 batch 0
