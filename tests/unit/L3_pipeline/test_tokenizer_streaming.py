# -*- coding: utf-8 -*-
"""
L3 Pipeline Tests: Streaming Tokenizer

对应模块: vit_pytorch.tokenizer_streaming

测试内容:
- HilbertPathCache - Hilbert 路径缓存机制
...
class TestMultiScalePatchEncoder2:
    """测试多尺度编码器.
...
class TestStreamingFractalTokenizerV3Full:
    ...
    def tokenizer_v3(self):
        """I98-1: 创建独立的 tokenizer 和 splitter."""
    ...
    def test_basic_forward(self, tokenizer_v3):
        """测试基础前向传播."""
        tokenizer, splitter = tokenizer_v3
        images = torch.randn(2, 3, 32, 32)

        # I98-1: 先调用 splitter，再调用 tokenizer
