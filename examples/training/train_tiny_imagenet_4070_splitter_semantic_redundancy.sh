...
# Splitter: SemanticRedundancySplitter (semantic similarity-based splitting)
...
#
# [SemanticRedundancySplitter Characteristics]
#   - Uses semantic redundancy detection for intelligent token splitting
#   - Computes feature similarity to identify redundant regions
#   - Learns to split tokens that contain semantically similar content
#   - Uses default hidden_dim=128, feature_dim=dim
#   - Best for: Tasks where semantic understanding is critical
#
# [KEY OPTIMIZATIONS]
...
#
# Expected: Top-1 Accuracy 55-68% | Time: ~25-35 hours
# Note: May require more compute for semantic similarity computation
# ============================================================================
...
uv run python -m src.training.train_fractal_vit \
  ...
  --splitter-type semantic_redundancy \
  --use-amp \
  --gradient-checkpoint \
  --compile \
  --channels-last \
