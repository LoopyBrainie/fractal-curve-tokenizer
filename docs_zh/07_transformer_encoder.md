...
### 7.2.3 残差门控（STAB-5, I34-10）

残差门控使用 **tanh** 激活调制注意力和 FFN 输出的贡献：
...
其中：
- 门控范围：$[0, 2]$（值 $>1$ 放大，$<1$ 衰减）
- $1 + g$ 确保残差连接始终打开（避免死路径）

**原理**：使用 $\tanh$ 而非 $2 \cdot \sigma$ 为接近 0 的值提供更稳定的梯度流，且加法形式 $(1 + g)$ 保证非零残差路径。

**V 形门控模式（I24-6）**：

基于训练观察：
- $d=0$（全局）：$w \approx 0.7$ - 抑制全局信息
- $d=1$：$w \approx 1.0$ - 保持原样
- $d=2$：$w \approx 0.85$ - 轻微抑制
- $d=3$（细粒度）：$w \approx 0.7$ - 抑制细节
